...
function addProduct($name, $description, $price, $stock, $category) {
    global $mysqli;
    $stmt = $mysqli->prepare('INSERT INTO products (name, description, price, stock, category) VALUES (?, ?, ?, ?, ?)');
...
function updateProduct($id, $name, $description, $price, $stock, $category) {
    global $mysqli;
    $stmt = $mysqli->prepare('UPDATE products SET name = ?, description = ?, price = ?, stock = ?, category = ? WHERE id = ?');
